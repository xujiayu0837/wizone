...
import org.apache.spark.sql.functions.{broadcast, lit, mean, round, explode, when, count, from_unixtime, max}
...
//      val dataDs = rdd.toDF().select(explode($"value").as("coll")).select($"coll.*").dropDuplicates().coalesce(4)
      val dataDs = rdd.toDF().dropDuplicates().coalesce(4)
      val blacklistDs = spark.read.parquet(PARQUETPATH.toString()).map(_.getString(0)).dropDuplicates().coalesce(4)
//      val filterDs = dataDs.filter($"rssi".gt(-90)).filter(!$"userMacAddr".isin(blacklistDs.collect():_*)).filter($"userMacAddr".substr(0, 6).isin(broadcastDs.collect():_*)).groupBy($"userMacAddr", $"ts", $"AP").agg(round(mean($"rssi")).alias("rssi")).orderBy($"userMacAddr", $"AP", $"ts").coalesce(4)
      dataDs.filter($"rssi".gt(-90)).filter(!$"userMacAddr".isin(blacklistDs.collect():_*)).filter($"userMacAddr".substr(0, 6).isin(broadcastDs.collect():_*)).groupBy($"userMacAddr", $"ts", $"AP").agg(round(mean($"rssi")).alias("rssi")).coalesce(4).createOrReplaceTempView("data30")
      ...
      val filterDs = spark.sql(sql0).coalesce(4)
      ...
      // 获取进出大楼人数
      val comeSql = s"SELECT data10.userMacAddr, data10.ts, data10.AP, data11.ts, data11.AP, data10.groupid FROM data10, data11 WHERE data10.userMacAddr = data11.userMacAddr AND data10.groupid = data11.groupid AND data11._id - data10._id = 1 AND data11.ts - data10.ts <= $THRESHOLD AND data10.AP = 'outside' AND data11.AP = 'inside'"
      val goSql = s"SELECT data10.userMacAddr, data10.ts, data10.AP, data11.ts, data11.AP, data10.groupid FROM data10, data11 WHERE data10.userMacAddr = data11.userMacAddr AND data10.groupid = data11.groupid AND data11._id - data10._id = 1 AND data11.ts - data10.ts <= $THRESHOLD AND data10.AP = 'inside' AND data11.AP = 'outside'"
...
//      spark.sql(sql2).show(false)
      ...
      val comeGoDs = comeCount.join(goCount, Seq("groupid"), "fullouter").coalesce(4)
      //      comeGoDs.show(false)
      ...
      val base = spark.read.jdbc("jdbc:mysql://127.0.0.1:3306/wibupt", tb0, prop)
//      base.show(2000, false)
      val joinDs = base.join(count, Seq("groupid"), "fullouter").coalesce(4)
      ...
      resDf.write.mode("append").jdbc("jdbc:mysql://127.0.0.1:3306/wibupt?verifyServerCertificate=false&useSSL=true", "realtime_statistic", prop)
    }
