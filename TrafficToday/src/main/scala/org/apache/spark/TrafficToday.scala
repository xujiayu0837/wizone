...
import org.apache.spark.sql.functions.{countDistinct, last, lit, to_date, window}

import scala.collection.mutable
import scala.collection.mutable.ArrayBuffer

/**
  * Created by xujiayu on 17/4/12.
  */
object TrafficToday {
  ...
  def insertTable(user: String, pwd: String, groupid: Int, count: Long): Unit = {
    ...
    try {
      ...
      val sql = "INSERT INTO traffic_today(groupid, daytraffic, monTime) VALUES(?, ?, ?)"
...
//      pstmt.setLong(3, getgelin(getToday()))
  ...
  def run(groupid: Int, args: Array[String], spark: SparkSession): Unit = {
    try {
      ...
      // 读取数据
      val multiPaths = readData(DATAPATH, 1, groupid.toString)
      ...
      val hadoopRdd = originRdd.asInstanceOf[NewHadoopRDD[LongWritable, Text]].mapPartitionsWithInputSplit((inputSplit, it) => {
        ...
        it.map(tup => {
          ...
          new StringBuilder(content.toString).append("|").append(locStr).toString
//          new StringBuilder(content.toString).append("|").append(apMacAddr).toString
      ...
      dataDs.persist()
      // 相同MAC地址只算一个用户
      val windowDs = dataDs.filter(to_date(dataDs("ts")).equalTo(lit(getToday()))).groupBy(window($"ts", "1 day", "1 day", timezoneFix).as("time")).agg(countDistinct("userMacAddr").as("count"))
      val resDs = windowDs.sort("time.start").select("time.start", "time.end", "count")
      // 测试
      //        resDs.show(truncate = false)
      try {
        // 获取计算结果
...
//        println("res: " + cnt)
  ...
  def main(args: Array[String]): Unit = {
    // 获取配置文件，并初始化spark
    // 测试
//    val spark = SparkSession.builder().config(new SparkConf()).appName("TrafficToday").master("local[*]").getOrCreate()
    val spark = SparkSession.builder().config(new SparkConf()).appName("TrafficToday").getOrCreate()
