...
object PeopleStatistic {
  ...
  val DESTPATH = "/tmp/idea_print/spark_1_2/170418_test_0"
...
//      val dataDs = dataDf.as[data]
//      dataDs.persist()
//      detectDs = dataDs.filter($"ts"<=new Timestamp(System.currentTimeMillis())).filter($"ts">new Timestamp(System.currentTimeMillis()-5*60*1000L)).groupBy("userMacAddr").agg(first("ts").as("ts"), first("groupid").as("groupid")).orderBy("ts")
//
//      //      println("detectDs:")
//      //      detectDs.show(2000, truncate = false)
//
//      val visitRecordRdd = spark.sparkContext.textFile(DESTPATH).cache()
//
//      //      println("visitRecordRdd: " + visitRecordRdd.count())
//
//      val detectDf = detectDs.select("userMacAddr")
//      val detectArr = detectDf.map(item=>{
//        item(0).asInstanceOf[String]
...
//      val visitArr = MyUtils.rdd2arr(visitRecordRdd)
//
////      val detectArr = detectDs.select("userMacAddr")
////
////      val temset = detectArr.map(line=>{
////        //        val line0 = stripChars(stripChars(line, "["), "]").split(",")(0)
////        //        line0.asInstanceOf[String]
////        line(0).asInstanceOf[String]
////      }).collect()
////      val l = temset.length
////      var detectArrBuf = new Array[String](l)
////      //      println("len: " + l)
////      detectArrBuf = temset
////      val visitSet = visitRecordRdd.map(line=>{
////        val line0 = MyUtils.stripChars(MyUtils.stripChars(line, "["), "]").split(",")(0)
////        line0.asInstanceOf[String]
////      }).collect()
////      var visitArrBuf = new Array[String](visitRecordRdd.map(line=>{
////        //        println("line: " + line)
////        //        println("line0: " + line(0) + " " + line(1))
////        //        println("test: " + stripChars(stripChars(line, "["), "]"))
////        val line0 = MyUtils.stripChars(MyUtils.stripChars(line, "["), "]").split(",")(0)
////        line0.asInstanceOf[String]
////      }).collect().length)
////      visitArrBuf = visitSet
//
//      val detectDiffVisit = detectArr.diff(visitArr)
//      //      var arr = new ArrayBuffer[DataFrame]
//      var strArr = new ArrayBuffer[String]
...
//      val newVisitArr = MyUtils.rdd2arr(newVisitRdd)
//
////      val newVisitSet = newVisitRdd.map(line=>{
////        val line0 = MyUtils.stripChars(MyUtils.stripChars(line, "["), "]").split(",")(0)
////        line0.asInstanceOf[String]
////      }).collect()
////      var newVisitArr = new Array[String](newVisitRdd.map(line=>{
////        val line0 = MyUtils.stripChars(MyUtils.stripChars(line, "["), "]").split(",")(0)
////        line0.asInstanceOf[String]
////      }).collect().length)
////      newVisitArr = newVisitSet
////      //      println("newVisitArr: " + newVisitArr)
//
//      val visitDiffDetect = newVisitArr.diff(detectArr)
//      //      println("visitDiffDetect: " + visitDiffDetect.toString)
//      val updateRdd = newVisitRdd.map(line=>{
...
//        println("No update")
//      }
//      //      val testRdd = sc.textFile("").flatMap(_.split(" ")).map((_, 1)).reduceByKey(_+_)
//      //  println("newVisitRdd: " + //.collect().toBuffer)
//      //      val extraRdd = newVisitRdd
//      Thread.sleep(5*1000L)
//      val cnt = updateRdd.filter(line=>{
//        val arr = MyUtils.stripChars(MyUtils.stripChars(line, "["), "]").split(",")
//        println("arr3: " + arr(3))
...
//
//      //      var cnt = 0
//      //      var temp = updateRdd.map(line=>{
//      //        val arr = stripChars(stripChars(line, "["), "]").split(",")
//      //        println("arr3: " + arr(3))
...
//      dataDs.unpersist()
      ...
      detectDs = refreshDetectDs(spark)
      val updateRdd = updateVisitRecord(detectDs, spark)
...
//    val spark = SparkSession.builder().config(new SparkConf()).appName("PeopleStatistic").getOrCreate()
    ...
    try {
//        val multiPaths = MyUtils.readData(DATAPATH, 1, "3")
//        val fc = classOf[TextInputFormat]
//        val kc = classOf[LongWritable]
//        val vc = classOf[Text]
//        val hadoopConf = new Configuration()
...
//
//        val sdf = new SimpleDateFormat("yyyy-MM-dd")
//        val detectDs = dataDs.filter($"ts" > new Timestamp(sdf.parse(MyUtils.getToday()).getTime)).filter($"ts" < new Timestamp(System.currentTimeMillis())).groupBy("userMacAddr").agg(first("ts").as("ts"), first("groupid").as("groupid")).orderBy("ts")
//        //      detectDs.show(2000, truncate = false)
//        val visitRecordDs = detectDs.withColumnRenamed("ts", "startTime").withColumn("endTime", lit(null))
//
////        Thread.sleep(10*1000L)
////        println("*"*50 + "initial dirdel run: " + "*"*50)
////        MyUtils.dirDel(new File(DESTPATH))
//
//        println("*"*50 + "initial run: " + "*"*50)
//        visitRecordDs.rdd.saveAsTextFile(DESTPATH)
